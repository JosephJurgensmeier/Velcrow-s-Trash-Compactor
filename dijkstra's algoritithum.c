...
void addPath(int source, int destination, int pathDistance,int pathDirection) //Function that creates paths between nodes
{
	...
	int i;
	for (i = 0;i == 4;i++)//Sets parking Flags
	{
		if(destination == parkingLots[i])
		{
				NODE_LIST[source].parkingFlag = true;
		}
...
task main()
{
	...
	{
	writeDebugStreamLine("the parking flags is checked on node %d = %b", i, NODE_LIST[i].parkingFlag);
}
}
